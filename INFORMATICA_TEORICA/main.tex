...
\begin{algorithm}[h]
	\caption{$\overline{n}=cod(\overline{P})$ con $\overline{P} \equiv$}
        \begin{algorithmic}[1]
        		 ...
        		 \State $\dots$
        		 \State $R_0 \leftarrow R_0+1$ \Comment{$y$ volte}
        		 \State $R_1 \leftarrow <R_1,R_0>$ \Comment{Creo input per $P$ $(*)$}
        		 \State $R_0 \leftarrow 0$ \Comment{Azzero $R_0$ $(**)$}
        		 \State $P$ \Comment{Richiamo $P$ su $R_1$}
        \end{algorithmic}
    \end{algorithm}
...
\begin{algorithm}[!h]
	\caption{$\hat{P} \equiv$}
        \begin{algorithmic}[1]
        		 \State $input(x)$
        		 \State $z:=U(x,x)$ \Comment{interpr. univ. $\phi_U(x,n)=\phi_n(x)$}
        \end{algorithmic}
    \end{algorithm}
